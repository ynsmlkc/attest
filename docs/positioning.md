...
- **SEP-0053 order signing — closed, 2026-09-12.** Their client signs each
  order with the trader's Stellar keypair before submitting it off-chain;
  the matching engine verifies that signature. Attest's
  `services/matching-engine` used to have no equivalent — anyone who knew a
  depositor's public address (visible on-chain from their `deposit()` call
  anyway) could submit an order *claiming* to be that depositor, and the
  engine had no way to tell. Fixed: `web/src/lib/orderSigning.ts` signs each
  order via Freighter's `signMessage()` before submission; the engine
  rebuilds the same canonical message and verifies it with
  `Keypair.verify()` in `services/matching-engine/index.js`, rejecting
  unsigned/mismatched/replayed orders with 401. Freighter's `signMessage()`
  was confirmed live (browser console, against `Keypair.verify`) to
  implement SEP-53 exactly — prefix `"Stellar Signed Message:\n"`, SHA-256,
  raw 64-byte ed25519 — before trusting it, not assumed from the SEP text
  alone. SEP-53 itself defines no replay defense, so a short `ts` window
  plus a seen-signatures cache was added on top. Not yet live on the
  deployed CVM (needs a rebuild + redeploy, held off per this session's
  cost policy) — this was still a real gap worth closing, independent of
