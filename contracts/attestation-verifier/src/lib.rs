...
impl AttestationVerifier {
    ...
    /// gets to.
    ///
    /// 2026-09-13 finding, fixed same day: once a quote has been submitted
    /// on-chain once, `payload`/`signature` are public — anyone can replay
    /// them here with a *different* `engine_address` and hijack an
    /// already-registered enclave_id, since the old check only required the
    /// *new* address's auth, never the existing one's. Fixed by requiring
    /// the currently-registered `engine_address` (if any) to also authorize
    /// the change — only the real operator, who alone holds that address's
    /// key, can rotate their own registration. First-ever registration of a
    /// fresh enclave_id is unaffected (nothing existing to protect yet); a
    /// residual, lower-severity gap remains there (see docs/positioning.md
    /// — front-running the *first* registration of a well-known enclave_id
    /// still needs binding `engine_address` into the quote's own signed
    /// REPORT_DATA field, not done here).
    pub fn register_verified_enclave(
        ...
        if let Some(existing) = env
            .storage()
            .persistent()
            .get::<_, VerifiedEnclave>(&DataKey::VerifiedEnclave(enclave_id.clone()))
        {
            existing.engine_address.require_auth();
        }
