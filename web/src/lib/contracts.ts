...
interface SettlementVaultApi {
  ...
  get_order_key(args: { party: string }): Promise<AssembledTransaction<Buffer | undefined>>;
  set_order_key(args: {
    party: string;
    key: Buffer;
  }): Promise<AssembledTransaction<unknown> & { signAndSend(): Promise<SentTransaction<unknown>> }>;
...
/**
 * The vault verifies each order's SEP-0053 signature on-chain, against an
 * ed25519 key the trader links to their Address once (`set_order_key`). For
 * a normal wallet that key is just the public key behind the G... address --
 * the same key Freighter's signMessage() signs with.
 */
export async function getOrderKey(party: string): Promise<Buffer | null> {
  const client = await vaultClient();
  const tx = await client.get_order_key({ party });
  return tx.result ?? null;
}

export async function registerOrderKey(party: string) {
  const client = await vaultClient(party);
  const tx = await client.set_order_key({
    party,
    key: Buffer.from(StrKey.decodeEd25519PublicKey(party)),
  });
  return tx.signAndSend();
}
