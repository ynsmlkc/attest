...
export async function getRealPriceHistory24h(symbol: string): Promise<PricePoint[]> {
  ...
  // allSettled, not all: one transient RPC failure among 21 independent
  // hourly lookups shouldn't blank the whole chart -- degrade to whatever
  // hours did resolve instead.
  const hourlySettled = await Promise.allSettled(
    hourlyOffsets.map((offset) => getReflectorPriceAt(symbol, nowSec - offset)),
  );
  const hourly = hourlySettled.map((r) => (r.status === "fulfilled" ? r.value : null));
